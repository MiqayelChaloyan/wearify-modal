const ButtonsTexts = {
    send: 'Send',
    next: 'Next',
    back: 'Back',
    tryOn: 'Try-On ME',
    ok: 'Ok',
    backScanning: '<  Back',
    info: 'i',
    yes: 'Yes',
    no: 'No'
};

const OptionsTexts = {
    upload: 'Upload',
    images: 'Images'
};

const Texts = {
    creatingModelImage: 'Creating model images, it’s expected to take 2 minutes.',
    uploadDescription: `Please upload 1 clear individual photos of the same model, ensuring that the model's facial features are fully visible in the pictures.`,
    loadingDescription1: 'Your task is in the queue..',
    scanningQR: '🖐️ Welcome to your \n virtual fitting room!',
    scanningText: 'Scan the QR code \n to open AR lens!',
    scann: '📽️   Scan QR with Mobile device',
    allow: '📷   Allow access to camera',
    tryOn: '👟   AR Try-On real time'
};

const ErrorMessege = {
    messege: 'You can upload a maximum of 10 images.'
};

const Titles = {
    height: 'HEIGHT',
    weight: 'WEIGHT',
    skin: 'SKIN',
    shape: 'SHAPE',
    selectSkin: 'SELECT SKIN & SHAPE',
    sizes: 'INPUT SIZE',
    ownModel: 'BE YOUR OWN MODEL',
    yourModel: 'BE YOUR OWN MODEL',
    age: 'AGE',
    gender: 'GENDER',
    removeImage: 'Are you sure you want to remove the image?'
};

const UnitsOfMeasurement = {
    cm: 'CM',
    in: 'IN'
};

const Gender = {
    female: 'FEMALE',
    male: 'MALE'
};

const ImagePaths = {
    logoUrl: 'https://drive.google.com/thumbnail?id=1vT4xy5UQYQ4s4AwPdnliDVkk6LKJvDw2',
    QRUrl: 'https://drive.google.com/thumbnail?id=1l_IPyjhrRdaXk9COy5ucTK7dNOb-BPgg'
};

const DeepARLink = 'https://www.figma.com/design/ZWZaBqAbrTZNLmcFQOtGrz/Untitled?node-id=0-1&t=Bp14r7lo9f8RKLoE-0'

export {
    ButtonsTexts,
    ImagePaths,
    DeepARLink,
    Texts,
    Titles,
    UnitsOfMeasurement,
    ErrorMessege,
    OptionsTexts,
    Gender
}